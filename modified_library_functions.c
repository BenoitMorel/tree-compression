 ...
 * compare_splits is used for comparing splits from different trees
 */
int compare_splits_with_lengths(pll_split_t s1,
                           pll_split_t s2,
                           pll_unode_t * node_to_s1,
                           pll_unode_t * node_to_s2,
                           unsigned int split_len)
{
  ...
  {
    if (s1[i] != s2[i]) {
      int same_split = (int) (s1[i] > s2[i]?1:-1);
      int same_length = (int) (node_to_s1->length == node_to_s2->length?1:-1);
      double dist = fabs(node_to_s1->length - node_to_s2->length);
      return (same_split == same_length?same_split:-1);
    }
  }
  return 0;
}
                                                       ...
                                                       unsigned int tip_count)
{
  ...
  for (s1_idx=0; s1_idx<split_count && s2_idx<split_count; ++s1_idx)
  {
    int cmp = compare_splits_with_lengths(s1[s1_idx], s2[s2_idx], splits_to_node1[s1_idx], splits_to_node2[s2_idx], split_len);
    if (!cmp)
    {
      equal++;
      s1_present[s1_idx] = 1;
    ...
    else
    {
      ...
      {
        while(++s2_idx < split_count &&
              (cmp = compare_splits_with_lengths(s1[s1_idx], s2[s2_idx], splits_to_node1[s1_idx], splits_to_node2[s2_idx], split_len)) > 0);
        if (!cmp)
        {
           equal++;
           s1_present[s1_idx] = 1;
                                                       ...
                                                       unsigned int tip_count)
{
  ...
  for (s1_idx=0; s1_idx<split_count && s2_idx<split_count; ++s1_idx)
  {
    ...
    if (!cmp)
    {
      if(fabs(splits_to_node1[s1_idx]->length - splits_to_node2[s2_idx]->length)<0.000001) {
          equal++;
      }
      //printf("equal: %i, s1_idx: %i, s2_idx: %i\n", equal, s1_idx, s2_idx);
      s2_idx++;
